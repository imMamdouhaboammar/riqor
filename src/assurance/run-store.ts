...
function validateEventInput(options: AppendRunEventOptions) {
  if (options.subject !== undefined) boundedText(options.subject, "event subject", 256);
  if (options.digest !== undefined && !digestPattern.test(options.digest)) {
    throw new Error("invalid event digest");
  }
  if (options.evidenceRefs && options.evidenceRefs.length > 32) {
    throw new Error("too many evidence references");
  }
  for (const reference of options.evidenceRefs ?? []) boundedText(reference, "evidence reference", 128);
...
async function withFileLock<T>(lockPath: string, action: () => Promise<T>, options: LockOptions = {}) {
...
function validateRun(value: unknown, identity: RepositoryIdentity, expectedRunId: string): RiqorRun {
  ...
  if (typeof run.goal !== "string" || typeof run.createdAt !== "string" || typeof run.updatedAt !== "string") {
    throw new Error("invalid run record");
  }
...
async function readRunFile(options: RunLocation) {
  ...
  const path = join(runDirectory(options.stateRoot, options.identity.rootDigest, options.runId), "run.json");
  try {
    return validateRun(parseJson(await readTextFile(path), "run"), options.identity, options.runId);
  } catch (error) {
...
async function appendEventLocked(
  ...
  await assertSafeRegularFile(eventsPath, true);
...
export async function readActiveRun(options: Omit<RunLocation, "runId">): Promise<RiqorRun | null> {
  validateRootDigest(options.identity.rootDigest);
...
export async function createRun(options: CreateRunOptions): Promise<RiqorRun> {
  ...
  await mkdir(runsDirectory(options.stateRoot, options.identity.rootDigest), { recursive: true, mode: 0o700 });

  return withFileLock(join(project, ".active.lock"), async () => {
    const active = await readActiveRun({ stateRoot: options.stateRoot, identity: options.identity });
    ...
    const parent = options.parentRunId
      ? await readRunFile({ stateRoot: options.stateRoot, identity: options.identity, runId: options.parentRunId })
      : null;
...
export async function appendRunEvent(options: AppendRunEventOptions): Promise<RiqorTraceEvent> {
  validateRunId(options.runId);
  const directory = runDirectory(options.stateRoot, options.identity.rootDigest, options.runId);
...
export async function transitionRun(options: TransitionRunOptions): Promise<RiqorRun> {
  validateRunId(options.runId);
  const directory = runDirectory(options.stateRoot, options.identity.rootDigest, options.runId);
  return withFileLock(join(directory, ".lock"), async () => {
    const run = await readRunFile(options);
...
export async function readRunEvents(options: RunLocation): Promise<RiqorTraceEvent[]> {
  const run = await readRunFile(options);
  const path = join(runDirectory(options.stateRoot, options.identity.rootDigest, options.runId), "events.jsonl");
...
export async function completeRun(options: CompleteRunOptions): Promise<RiqorRun> {
  validateRunId(options.runId);
  const directory = runDirectory(options.stateRoot, options.identity.rootDigest, options.runId);
  const completed = await withFileLock(join(directory, ".lock"), async () => {
    const run = await readRunFile(options);
    if (run.status === "verification-pending") throw new Error("verification is still pending");
