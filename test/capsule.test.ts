...
import { classifyTask, createCapsule, destroyCapsule, selectedCapabilities, selectedHarnessPath } from "../src/capsule";
...
describe("task classifier", () => {
  test("selects the database profile from task meaning", () => {
    ...
    expect(selectedHarnessPath("Repair this PostgreSQL schema").id).toBe("architecture-conformance");
  ...
  test("selects review before the generic engineering fallback", () => {
    ...
    expect(selectedHarnessPath("Audit the previous completion claim").id).toBe("independent-review");
  ...
  test("keeps the engineering fallback bounded", () => {
    expect(selectedCapabilities("Implement the parser feature and its coverage").map(({ name }) => name)).toEqual([
...
test("capsule is owner-only, links auth and selected skills, and is removable", async () => {
  ...
  const capsule = await createCapsule({
    ...
    path: selectedHarnessPath("trace this bug to its root cause"),
  });
  ...
  const instructions = await readFile(join(capsule, "AGENTS.md"), "utf8");
  expect(instructions).toContain("Harness path: evidence-loop");
  expect(instructions).toContain("Reproduction or failing check before the change");
  expect(instructions).not.toContain("secret-auth");
